...
#pragma omp parallel for private(btemp)
    ...
    A.c5=c5;
    A.c6=c6;
    int usersize, userno1,userno2,tt,k; 
    double off_diff=0,Osize=0;
    ...
    for (i=0; i<p; ++i) {
		...
		for (l=0; l<usersize; ++l) {
			...
			for (tt=l+1; tt<usersize; ++tt) {
				...
				off_diff+=abs(Omegais2[userno1](i,i)-Omegais2[userno2](i,i));
			}
			userno2=itemmore_idle[i];
			off_diff+=(n-usersize)*abs(Omegais2[userno1](i,i)-Omegais2[userno2](i,i));
		}
	} 
#pragma omp parallel for private(l,usersize,i,j,userno1,userno2,tt) reduction(+:off_diff,Osize)
	for (k=0; k<rlv.size();++k) {
		...
		for (l=0; l<usersize; ++l) {//l should be private
			...
			for (tt=l+1; tt<usersize; ++tt) {
				...
				off_diff+=abs(Omegais2[userno1](i,j)-Omegais2[userno2](i,j));
			}
			userno2=rlv_idle[k];
			off_diff+=(n-usersize)*abs(Omegais2[userno1](i,j)-Omegais2[userno2](i,j));
			Osize+=abs(Omegais2[userno1](i,j));
		}
		Osize+=(n-usersize)*abs(Omegais2[userno2](i,j));
	}
	A.off_diff=off_diff;A.Osize=Osize;//cal_offdiff_prop(trin,*A.Omegais);
...
#pragma omp parallel for
    ...
    vector<MatrixXd> &Uis=C.Uis; //reference, refer to the same variable
...
#pragma omp parallel for 
        ...
        obj1=cal_obj_prop_struct_6(c,trin,trip,betadoc_pre,alphadoc_pre,Omegaisdoc_pre,Sis,A);//the function value admm needs to minimize, admm check its decrease
        tstart3=time(0);
...
#pragma omp parallel for private(movieuse,Omegaissub,alphause,useri)
					...
					//inexact admm for solve beta
#pragma omp parallel for private(atemp)
					for (i=0; i<n; ++i) {
						tempB[i]+=tempB1[i];
...
#pragma omp parallel for private(j,btemp)
					...
					//D.theta2=theta2;
					//D.u2=u2;
					while (alpha_iter_inner<1) {//->1, was C.maxIter, regard alpha_j as a block, so it's beta_1,beta_2...beta_n,alpha_1,alpha_2,...alpha_m, Omega1,Omega2,Omega_n...
	...
	#pragma omp parallel for private(btemp,Tm,j,mtemp)
					for (i=0; i<n; ++i) {
						...
						pretilde[i]=C.x.user[i].rating-pretilde[i];
						Sis[i]=pretilde[i]*(pretilde[i].transpose());
						//cout<<"User "<<i<<" Si is "<<endl;
						//cout<<Sis[i]<<endl<<endl<<endl;
						mtemp=Omegais2[i];
					...
					if(admm_iter<100 and iter_ori>10) break;
				}
            }
            else{
				MatrixXd Tm;
				...
				MatrixXd solu=C.users*mualpha2+mubeta2*C.movie.transpose();
				C.log_det=VectorXd::Zero(n);
#pragma omp parallel for private(btemp,Tm,j)
				for (i=0; i<n; ++i) {
					//MatrixXd alphause=matrix_colsub(mualpha2,C.x.user[i].item);
					//MatrixXd movieuse=matrix_rowsub(C.movie,C.x.user[i].item);
					//VectorXd useri=C.users.row(i);
...
#pragma omp parallel for private(i,j,temp,atemp,btemp,a)
                ...
                MatrixXd::Index maxRow, maxCol;
  				maxbeta=absm(mubeta2-betapre).maxCoeff(&maxRow, &maxCol);
...
#pragma omp parallel for private(mtemp)
		...
		#pragma omp parallel for private(l,usersize,i,j,userno1,userno2,tt) reduction(+:off_diff,Osize)
			...
			A.off_diff=off_diff;A.Osize=Osize;
        ...
        if (obj_doc1-obj_doc2<0.005) {//&&doc_iter>0//0.01 changed to 0.005//added two more conditions//&&doc_iter>10&&(obj_doc2<obj_doc1)
            break;  
        }
        ++doc_iter;
        obj_doc1=obj_doc2;
