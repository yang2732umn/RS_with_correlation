...
result Cluster_TLP_scale_prop_size5(double c,Cluster_TLP_p_scale_para2 &C){//unparallel version
...
#pragma omp parallel for
    ...
    while (doc_iter<C.maxIter){
        vector<vector<int>> diffA(p*(p-1)/2),diffB(n*(n-1)/2);//redefine diffA and diffB every doc_iter
        betadoc=mubeta2;
        alphadoc=mualpha2;
        Omegaisdoc=Omegais2;
        admm_iter=0;
        tstart1=time(0);
        involvedB=VectorXi::Zero(n);//construct diffB
        for(i=0;i<n-1;++i){//diffB is in order 12,13,14,...1n; 23,24,25...2n;...(n-1)n
            for (j=i+1; j<n; ++j) {
                l=i*(n-(i+1)*0.5)+j-i-1;
                vector<int> diff1;
                for (int tt=0; tt<mdim; ++tt) {
                    if (abs(mubeta2(i,tt)-mubeta2(j,tt))<=C.tau) {
                        diff1.push_back(tt);
                    }
                }
                if(diff1.size()>0){
                    diffB[l]=diff1;
                    ++involvedB[i];++involvedB[j];
                }
            }
        }
        //in this version, diffB being empty is not a problem anymore， because have the c quadratic term
        ...
        for(i=0;i<p-1;++i){
            for (j=i+1; j<p; ++j) {
                ...
                for (int tt=0; tt<udim; ++tt) {
                    if (abs(mualpha2(tt,i)-mualpha2(tt,j))<=C.tau) {
                        diff1.push_back(tt);
                    }
                }
                if(diff1.size()>0){
                    diffA[l]=diff1;
                    ++involvedA[i];++involvedA[j];//cannot parallize because of this line
                }
            }
        }
        
        //construct Osize
#pragma omp parallel for private(item1,item2,usersize,M,tt,userno)
        for(i=0;i<rlv.size();++i){
            item1=rlv[i].item1;
            item2=rlv[i].item2;
            usersize=rlv[i].userno.size();
            M.clear();
            for (tt=0; tt<usersize+1; ++tt) {
                userno=tt<usersize?rlv[i].userno[tt]:rlv_idle[i];
                if(abs(Omegais2[userno](item1,item2))<=C.tau){
        ...
        vector<int> judge_diag(p,0), judge_offdiag(rlv.size(),0);//initialize to 0, in fact keeps # of constraints
        //only edits when usersize>2
#pragma omp parallel for private(l,usersize,userno1,userno2,cstrfdi1,cstrsdi1,cstrno,tt,pairtemp)
        for (i=0; i<p; ++i) {
        	cstrfdi1.clear();cstrsdi1.clear();//must do this, otherwise, only resize will keep the previous elements
            usersize=itemmore[i].user.size();
            if (usersize==1) {
                //because of the push_back(), cannot parallelize
                userno1=itemmore[i].user[0];
                userno2=itemmore_idle[i];
                if (abs(Omegais2[userno1](i,i)-Omegais2[userno2](i,i))<=C.tau){
            ...
            if (usersize>1){
                ...
                for (l=0; l<usersize; ++l) {//l should be private
                    ...
                    for (tt=l+1; tt<usersize+1; ++tt) {
                        userno2=tt<usersize?itemmore[i].user[tt]:itemmore_idle[i];
                        if (abs(Omegais2[userno1](i,i)-Omegais2[userno2](i,i))<=C.tau){
                            pairtemp(1)=cstrno;
                            pairtemp(0)=tt;
...
/* 
#pragma omp parallel for private(l,usersize,i,j,userno1,userno2,cstrno,cstrfdi1,cstrsdi1,userno,tt,pairtemp)
        for (k=0; k<rlv.size();++k) {
            usersize=rlv[k].userno.size();
            i=rlv[k].item1;
            j=rlv[k].item2;
            if (usersize==1) {
                ...
                userno2=rlv_idle[k];
                if(abs(Omegais2[userno1](i,j)-Omegais2[userno2](i,j))<=C.tau){
                    cstrfdi1.resize(2),cstrsdi1.resize(2);
                    pairtemp(1)=0;
                    pairtemp(0)=1;
                    cstrfdi1[0].push_back(pairtemp);
            ...
            if(usersize>1){
                ...
                for (l=0; l<usersize; ++l) {//l should be private
                    ...
                    for (tt=l+1; tt<usersize+1; ++tt) {
                        userno2=tt<usersize?rlv[k].userno[tt]:rlv_idle[k];//correct
                        if(abs(Omegais2[userno1](i,j)-Omegais2[userno2](i,j))<=C.tau){
                            pairtemp(1)=cstrno;
                            pairtemp(0)=tt;
 ...
 */
 	#pragma omp parallel for private(l)
        for (int k=0; k<rlv.size();++k) {
            ...
            int j=rlv[k].item2;
            if (usersize==1) {
                int userno1=rlv[k].userno[0];
                int userno2=rlv_idle[k];
                if(abs(Omegais2[userno1](i,j)-Omegais2[userno2](i,j))<=C.tau){
                    vector<vector<Vector2i>> cstrfdi1(2),cstrsdi1(2);
                    Vector2i pairtemp;
                    pairtemp(1)=0;
                    pairtemp(0)=1;
                    cstrfdi1[0].push_back(pairtemp);
            ...
            if(usersize>1){
                vector<vector<Vector2i>> cstrfdi1(usersize+1),cstrsdi1(usersize+1);
                int cstrno=0;
                for (l=0; l<usersize; ++l) {//l should be private
                    int userno1=rlv[k].userno[l];
                    for (int tt=l+1; tt<usersize+1; ++tt) {
                        int userno2=tt<usersize?rlv[k].userno[tt]:rlv_idle[k];//correct
                        if(abs(Omegais2[userno1](i,j)-Omegais2[userno2](i,j))<=C.tau){
                            Vector2i pairtemp;
                            pairtemp(1)=cstrno;
                            pairtemp(0)=tt;
        ...
        tend1=time(0);
        time1=difftime(tend1,tstart1);
        cout<<"Calculating obj_abs1 cost "<<time1<<" seconds."<<endl;
...
#pragma omp parallel for private(l)
                ...
                /*
                 while (beta_iter_inner<C.maxIter){//update beta unparallelly
                 double obj_inner0=cal_obj_abo(C.rho,C.rho2,E,diffA,diffB,theta2,u2,theta,u,Zis,Uis);
                 //cout<<"Before changing beta, obj_inner="<<obj_inner0<<endl;
                 MatrixXd mubeta1_inner2=mubeta2;
                 for(i=0;i<n;++i){
                 tempB[i]=VectorXd::Zero(mdim);
                 MatrixXd mtemp=MatrixXd::Zero(n,mdim);
                 #pragma omp parallel for private(l)
                 for (j=0; j<n; ++j) {
                 if(j==i) continue;
                 if (j<i) l=j*(n-(j+1)*0.5)+i-j-1;
                 ...
                 if (diffB[l].size()>0) {
                 mtemp.row(j)=mubeta2.row(j);//not mubeta1_inner2
                 }
                 }
                 tempB[i]+=mtemp.colwise().sum();
                 tempB[i]*=rho_beta;
                 tempB[i]+=tempB0[i];
                 tempB[i]+=tempB1[i];
                 mubeta2.row(i)=mtempB[i]*tempB[i];
                 ...
                 //cout<<"After change beta row "<<i<<", obj_inner="<<obj_inner<<endl;
