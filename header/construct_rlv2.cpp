...
vector<Omega_1rlvmore> construct_rlv2(int p,const vector<rated_user> &user){//rlv is in order, only contain those item pairs rated simultaneously by at least one user
    int n=user.size();
    vector<Omega_1rlvmore> rlv(p*(p-1)/2);
    Omega_1rlvmore rlv1;
    int good=0;
    int l=0;
        ...
        for (good=0;good<secno.size();++good) {
            ...
            for(int k=0;k<n;++k){
                ...
                if((findi!=-1)&&(findj!=-1)){
                    ...
                    rlv1.firstrating.push_back(user[k].rating[findi]);
                    rlv1.secrating.push_back(user[k].rating[findj]);
                }
            }
            if(count==0){
