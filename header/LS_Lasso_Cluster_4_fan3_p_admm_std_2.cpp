...
result LS_Lasso_Cluster_4_fan3_p_admm_std_2(rated_user_and_item x, MatrixXd movie, MatrixXd users, MatrixXd mualpha1, MatrixXd mubeta1, double lambda1, double rho,double Tol, int maxIter,int factor){
    ...
    two_lambda_struct A;
    A.lambda1=c1*lambda1/c4/factor;//for beta, use lambda1/2
    ...
    double lambda_beta=c1*lambda1/c4/factor;//for beta, use lambda1/2
...
#pragma omp parallel for private(j,l,temp,atemp,btemp,a,b)
            for (i=0; i<(n-1); ++i) {
                for (j=(i+1); j<n; ++j) {
                    ...
                    btemp=(*u).col(l);
                    atemp=atemp+btemp;
                    a=ST_vec(atemp,lambda1/(2*factor*rho));//also changed here
                    atemp=a-(*theta).col(l);
                    (*theta).col(l)=a;
...
#pragma omp parallel for private(j,l,btemp,b)
        ...
        {   cout<<"alpha_iter is "<<alpha_iter<<endl;
            ...
            obj2=cal_obj2_2lambda_struct(A)/c1*100;
            cout<<"obj="<<obj2<<endl;
            if (abs(obj1-obj2)<1e-5&&iter>620) {
                break;
            }
            else{
                obj1=obj2;
