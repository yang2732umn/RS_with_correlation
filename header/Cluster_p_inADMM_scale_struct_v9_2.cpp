...
#pragma omp parallel for private(j,l,atemp) reduction(+:off_diff)
            for(i=0;i<p;++i){
                ...
                atemp=solve21_ama2_prop(atemp,n-usersize,rho_Zdiag,1000,2e-4);//was 1e-4 previously
            ...
            for (int k=0; k<rlv.size();++k) {
                ...
                atemp=solve21_ama2_prop(atemp,n-usersize,rho_Zdiag/2,1000,2e-4);//was 1e-4 previously
                //cout<<"atemp="<<atemp.transpose()<<endl;
...
#pragma omp parallel for private(mtemp)
            ...
            cout<<"admm iter is "<<admm_iter<<", doc_iter="<<doc_iter<<", abs(obj1-obj2)="<<abs(obj1-obj2)<<", tttalpha="<<tttalpha<<", tttbeta="<<tttbeta<<endl;
