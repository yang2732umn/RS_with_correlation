...
VectorXd solve21_hock2(VectorXd &A,double lambda){
    ...
    for(int iter=1;iter<=K-1;++iter){
        ...
        VectorXd newcg=VectorXd::Zero(temp.size());
        
        for (int i=0; i<temp.size(); ++i) {//left in temp are all representatives of each class
        ...
        /*newc=VectorXd::Zero(K);
         ...
         cout<<"newc second is "<<newc.transpose()<<endl;*/
    }
    betas=A-lambda*newc;
