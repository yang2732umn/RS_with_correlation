...
vector<Omega_1rlv_simple> construct_rlv_simple(int p,const vector<rated_user> &user){//rlv is in order, only contain those item pairs rated simultaneously by at least one user
    int n=user.size();
    vector<Omega_1rlv_simple> rlv(p*(p-1)/2);
    Omega_1rlv_simple rlv1;
    int good=0;
    int l=0;
